 ...
 * with the same name and .cc extension.3218
 */
...
struct PLAYER_NAME : public Player {
   ...
   */
	...
	double distance (Pos p1, Pos p2) {
		...
		double distB = sqrt(pow(80 - abs(p1.i + p2.i), 2) + pow(p1.j - p2.j, 2));
	...
	// bool imBeingStalked(Pos p, Unit &u) { //Igual priorizar hellhound según distancia pero si hay furyan muy cerca priorizar BFS? 
	...
	bool imBeingStalked_v2(Pos p, Unit &u2) { //tener en cuenta segundo tipo distancia si atraviesa derecha
		set<Pos> visited;
		queue<Pos> Q;
		Q.push(p);
		int dist;
		while (not Q.empty()) {
			...
			if (isPassable(currentPos) and c.id != -1) {
				Unit tmpUnit = unit(c.id);
				if (tmpUnit.type == Hellhound and distance(tmpUnit.pos, p) < 5) {
					u2 = tmpUnit;
					return true;
				}
				else if (tmpUnit.type == Furyan and distance(tmpUnit.pos, p) < 3) {
					u2 = tmpUnit;
					return true;
				}
			}
